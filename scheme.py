...
def remove_outliers_with_clipping(scores):
    ...
    clipped_scores = np.clip(scores, lower_bound, upper_bound)
...
if st.button("Calculate Weights and Check Consistency"):
    ...
    if priority_vector is not None and matrix is not None:
        ...
        if consistency_ratio < 0.1:
            mutual_funds = pd.read_csv("ALL_SCHEMES_DATA.csv")
            
            scores = mutual_funds[criteria].dot(priority_vector)
            # scores = remove_outliers_with_clipping(scores)
            mutual_funds["Score"] = scores
            print(scores)
            ranked_funds = mutual_funds.sort_values(by="Score", ascending=False)
            
            st.success(f"The consistency ratio is acceptable ({consistency_ratio:.2f}).")
            
            st.write("Top Mutual Fund Schemes:")
            showing_funds = ranked_funds.drop_duplicates(subset="scheme_code")
            showing_funds = showing_funds.head(10)
            st.dataframe(showing_funds)
            
            print(showing_funds["Score"].nunique())
            unique_schemes = showing_funds.drop_duplicates(subset="scheme_code")
            
            for i in range(len(unique_schemes)):
                scheme_name = unique_schemes.iloc[i]["scheme_name"]
                
                scheme_nav_data = mutual_funds[mutual_funds["scheme_name"] == scheme_name]["nav"]
